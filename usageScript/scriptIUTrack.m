% - a movieParam struct must be define (eg in a detection script scriptDetectPointSourceForUTrack.m)
% - a movieInfo struct resulting from object detection must be loaded in memory

%% input output
saveResults.dir =  ['../res/']; %directory where to save input and output
saveResults.filename='iimm_tracking.mat'
...
parameters.kalmanInitParam.searchRadiusFirstIteration = 8; %Kalman filter initialization parameters.
...
[tracksFinal,kalmanInfoLink,errFlag] = trackCloseGapsKalmanSparse(movieInfo,...
                                                  ...
                                                  saveResults,verbose,'watch_KF_iter',watch_KF_iter,...
                                                  'movieParam',movieParam,'revMovieParam',revMovieParam,...
                                                  'iter_link_nb',iter_link_nb);
