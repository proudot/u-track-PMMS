...
for p = 1:np
    ...
    if (xi(p)>w4 && xi(p)<=nx-w4 && yi(p)>w4 && yi(p)<=ny-w4)
        ...
        if npx >= 10 % only perform fit if window contains sufficient data points
            ...
            if(showDebug)
                subplot(1,4,1)
                imshow(mat2gray(window))
                hold on
                scatter((size(window,2)/2),(size(window,1)/2),'b+');
                scatter((size(window,2)/2)+x(p)-xi(p) ,(size(window,1)/2)+y(p)-yi(p),'go');
                scatter((size(window,2)/2)+dx,(size(window,1)/2)+dy,'ro');
                hold off
                subplot(1,4,2)
                imshow(mat2gray(window))
                subplot(1,4,3)
