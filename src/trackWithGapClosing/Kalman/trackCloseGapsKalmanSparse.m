...
%                      Optional. Default: 1.
%       varargin : 
%           'iter_link_nb'  : Number of tracking iterations 0 mean one track pass. 
%                             n mean n iteration
%           'watch_KF_iter' : Display Kalman filtering results during computation.
%           'movieParam'    : movieParam used for frame-to-frame linking display
%           'revMovieParam' : movieParam used for reverse track frame-to-frame linking display
%
...
p = inputParser;
% Number of iteration used in tracking (works for U-track and Iterative U-track)
addOptional(p,'iter_link_nb',1);        
addOptional(p,'watch_KF_iter',0);
addOptional(p,'movieParam',0);
addOptional(p,'revMovieParam',0);
addOptional(p,'figureSize',[]);
parse(p,varargin{:});
...
if (p.Results.watch_KF_iter && isstruct(p.Results.movieParam))
    firstImageFile=strcat(p.Results.movieParam.imageDir,p.Results.movieParam.filenameBase,...
                          sprintf('%04d ', ...
                                  p.Results.movieParam.firstImageNum),'.tif');
    startend=[p.Results.movieParam.firstImageNum p.Results.movieParam.lastImageNum];

end

if (p.Results.watch_KF_iter && isstruct(p.Results.revMovieParam))
    firstImageFileRev=strcat(p.Results.revMovieParam.imageDir,p.Results.revMovieParam.filenameBase,...
											sprintf('%04d ',p.Results.revMovieParam.firstImageNum),'.tif');                                        
    startend=[p.Results.revMovieParam.firstImageNum p.Results.revMovieParam.lastImageNum];
end


% parameters used for frame-to-frame linking vizualization
...
if selfAdaptive
    ...
    if verbose
     ...
     [tracksFeatIndxLink,tracksCoordAmpLink,kalmanInfoLink,nnDistLinkedFeat,...
        linkingCosts,errFlag] = linkFeaturesKalmanSparse( ...
        movieInfo,costMatrices(1).funcName,costMatrices(1).parameters, ...
        kalmanFunctions,probDim,[],[],verbose);
    clear dummy
    if (p.Results.watch_KF_iter && isstruct(p.Results.movieParam))
        movieName='first_track_forward';
        overlayKFSchemeLink(movieInfo,tracksFeatIndxLink,kalmanInfoLink,costMatrices(1).parameters, startend,dragtailLength,...
                            ...
                            colorTracks,firstImageFile,dir2saveMovie,'displayCutoff',displayCutoff,...
                            'displayPred',displayPred,'figureSize',p.Results.figureSize);
    end

    
    for iter= 1 : p.Results.iter_link_nb
        ...
        [tracksFeatIndxLink,dummy,kalmanInfoLink,dummy,linkingCosts] = linkFeaturesKalmanSparse(...
        ...
        if (p.Results.watch_KF_iter && isstruct(p.Results.revMovieParam))
            movieName= ['iter_'  int2str(iter) '_reverse'];
            overlayKFSchemeLink(movieInfo(end:-1:1),tracksFeatIndxLink,kalmanInfoLink,...
                                costMatrices(1).parameters, startend,dragtailLength,...
                                saveMovie,movieName,filterSigma,classifyGaps,highlightES,showRaw,...
                                imageRange,onlyTracks,classifyLft,diffAnalysisRes,intensityScale,...
                                colorTracks,firstImageFileRev,dir2saveMovie,'displayCutoff',displayCutoff,...
                                'displayPred',displayPred,'figureSize',p.Results.figureSize);
        end
        ...
        kalmanInfoLinkRev=kalmanInfoLink;
        [tracksFeatIndxLink,tracksCoordAmpLink,kalmanInfoLink,nnDistLinkedFeat,...
        linkingCosts,errFlag]= linkFeaturesKalmanSparse( ...
        movieInfo,costMatrices(1).funcName,costMatrices(1).parameters, ...
        kalmanFunctions,probDim,kalmanInfoLink,linkingCosts,verbose);
        ...
        if (p.Results.watch_KF_iter && isstruct(p.Results.movieParam))
            movieName= ['iter_'  int2str(iter) '_forward'];
            overlayKFSchemeLink(movieInfo,tracksFeatIndxLink,kalmanInfoLink,...
                                costMatrices(1).parameters, startend,dragtailLength,...
                                saveMovie,movieName,filterSigma,classifyGaps,highlightES,showRaw,...
                                imageRange,onlyTracks,classifyLft,diffAnalysisRes,intensityScale,...
                                colorTracks,firstImageFile,dir2saveMovie,'displayCutoff',displayCutoff,...
...
if any(trackStartTime > 1) && any(trackEndTime < numFramesEff)
    ...
    if any(isfinite(nonzeros(costMat)))
        ...
        for iTrack = 1 : numTracksCG
            ...
            seqOfEvents = ...
                [segmentStartTime ones(seedLength,1) (1:seedLength)' trackSeedConnect(:,3); ...
                 segmentEndTime 2*ones(seedLength,1) (1:seedLength)' trackSeedConnect(:,2)];
