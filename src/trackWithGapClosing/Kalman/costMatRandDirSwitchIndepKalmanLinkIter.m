function [costMat,propagationScheme,kalmanFilterInfoFrame2,nonlinkMarker, ...
          errFlag] = ...
    costMatRandDirSwitchIndepKalmanLinkIter(movieInfo,kalmanFilterInfoFrame1,...
...
%             .filterInfoPrev     : Exploiting prediction of the previous track.
%
...
if nargin ~= nargin('costMatRandomDirectedSwitchingMotionLink')
    disp('--costMatRandomDirectedSwitchingMotionLinkIter: Incorrect number of input arguments!');
    errFlag  = 1;
    return
end
...
numPred=numSchemes;
filterInfoPrev=costMatParam.filterInfoPrev;

if ~isempty(filterInfoPrev)
    numPred=2*numSchemes-1;
end

%reserve memory for "kalmanFilterInfoframe2"
kalmanFilterInfoFrame2 = struct('stateVec',nan(numFeaturesFrame1,vecSize,numPred),...
                                'stateCov',nan(vecSize,vecSize,numFeaturesFrame1,numPred),...
                                'obsVec',nan(numFeaturesFrame1,probDim,numPred));
...
for iFeature = 1 : numFeaturesFrame1
    ...
    for iScheme = 1 : numSchemes
        ...
        stateOld = kalmanFilterInfoFrame1.stateVec(iFeature,:,iScheme)';
        stateCovOld = kalmanFilterInfoFrame1.stateCov(:,:,iFeature,iScheme);
        noiseVar = abs(kalmanFilterInfoFrame1.noiseVar(:,:,iFeature,iScheme));
    ...
    if ~isempty(filterInfoPrev)
        % Consider the scheme selected for the same link during the last tracking round
        bestPastScheme=filterInfoPrev.scheme(iFeature,1);

        % Conisder the prediction only if the past scheme is was a new prediction
        % (and not a prediction from an anterior track)
        if ((bestPastScheme>0)&&(bestPastScheme<=numSchemes))
            
            %get state vector and its covariance matrix of feature in 1st frame
            stateOld = filterInfoPrev.stateVec(iFeature,:,bestPastScheme)';
            stateCovOld = filterInfoPrev.stateCov(:,:,iFeature,bestPastScheme);
            noiseVar = filterInfoPrev.noiseVar(:,:,iFeature,bestPastScheme);    
            
            schemeIdx=numSchemes- mod(bestPastScheme,  numSchemes);

            %predict state vector of feature in 2nd frame
            stateVec = transMat(:,:,schemeIdx)*stateOld;
            
            %predict state covariance matrix of feature in 2nd frame
            stateCov = transMat(:,:,schemeIdx)*stateCovOld* ... 
                transMat(:,:,schemeIdx)' ...
                + noiseVar;
            ...
            %save information in kalmanFilterInfoFrame2
            kalmanFilterInfoFrame2.stateVec(iFeature,:,numSchemes+schemeIdx) = stateVec';
            kalmanFilterInfoFrame2.stateCov(:,:,iFeature,numSchemes+schemeIdx) = stateCov;
            kalmanFilterInfoFrame2.obsVec(iFeature,:,numSchemes+schemeIdx) = obsVec';
        end
    end    
...
notFirstAppearance = squeeze(kalmanFilterInfoFrame1.noiseVar(1,1,:,numSchemes+1)) >= 0;

%get the Kalman standard deviation of all features in frame 1
kalmanStd = sqrt(probDim * abs(squeeze(kalmanFilterInfoFrame1.noiseVar(1,1,:,numSchemes+1))));


%copy brownStdMult into vector
stdMultInd = repmat(brownStdMult,numFeaturesFrame1,1);

%if loc al density information is used to expand search radius ...
if useLocalDensity
...
% If the selected prediction stems from the last track round then adapt the search radius accordingly.
if ~isempty(filterInfoPrev)
    kalmanStdPast = sqrt(probDim * abs(squeeze(filterInfoPrev.noiseVar(1,1,:,numSchemes+1))));
    searchRadiusPast = stdMultInd .* kalmanStdPast;
    searchRadiusPast((searchRadiusPast>maxSearchRadius)&notFirstAppearance) = maxSearchRadius;
    searchRadiusPast((searchRadiusPast<minSearchRadius)&notFirstAppearance) = minSearchRadius;
    searchRadiusPast = repmat(searchRadiusPast,1,numFeaturesFrame2);
    searchRadius(propagationScheme>numSchemes)=searchRadiusPast(propagationScheme>numSchemes);
end
