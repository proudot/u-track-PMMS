...
if selfAdaptive
    ...
    eval(['kalmanFilterInfo = ' kalmanFunctions.reserveMem ...
          '(numFrames,numFeatures,probDim,costMatParam);']);
...
if selfAdaptive
    ...
    if usePriorInfo %use a priori information if available
        ...
        % this statement allow to preserve compatibility with non-online variance estimation,
        if(isfield(filterInfoPrev(1),'stateNoiseRSS')) 
            kalmanFilterInfo(1).stateNoiseRSS = filterInfoPrev(1).stateNoiseRSS;
            kalmanFilterInfo(1).stateNoiseMean = filterInfoPrev(1).stateNoiseMean;
            kalmanFilterInfo(1).stateNoiseNum = filterInfoPrev(1).stateNoiseNum;
        end 
    else
        ...
        if(isfield(filterInit,'stateNoiseRSS'))
            kalmanFilterInfo(1).stateNoiseRSS = filterInit.stateNoiseRSS;
            kalmanFilterInfo(1).stateNoiseMean = filterInit.stateNoiseMean;
            kalmanFilterInfo(1).stateNoiseNum = filterInit.stateNoiseNum;
        end 
    end
...
for iFrame = 1 : numFrames-1
    ...
    if numFeaturesFrame1 ~= 0 %if there are features in 1st frame
        ...
        if numFeaturesFrame2 ~= 0 %if there are features in 2nd frame
            ...
            % Iterative Kalman Filtering cost matrix build need filter info from previous track. 
            % We
            if ~isempty(filterInfoPrev)
                costMatParam.filterInfoPrev=filterInfoPrev(iFrame);    
            else
                costMatParam.filterInfoPrev=[];
            end 
            ...
            if any(costMat(:)~=nonlinkMarker) %if there are potential links
                ...
                if selfAdaptive
                    ...
                    if usePriorInfo %use a priori information if available
                        ...
                        if(isfield(filterInfoPrev(iFrame+1),'stateNoiseRSS'))
                            kalmanFilterInfo(iFrame+1).stateNoiseRSS = filterInfoPrev(iFrame+1).stateNoiseRSS;
                            kalmanFilterInfo(iFrame+1).stateNoiseMean = filterInfoPrev(iFrame+1).stateNoiseMean;
                            kalmanFilterInfo(iFrame+1).stateNoiseNum = filterInfoPrev(iFrame+1).stateNoiseNum;
                        end 
                    else
                        eval(['[filterInit,errFlag] = ' kalmanFunctions.initialize ...
                        ...
                        if(isfield(filterInit,'stateNoiseRSS'))
                            kalmanFilterInfo(iFrame+1).stateNoiseRSS = filterInit.stateNoiseRSS;
                            kalmanFilterInfo(iFrame+1).stateNoiseMean = filterInit.stateNoiseMean;
                            kalmanFilterInfo(iFrame+1).stateNoiseNum = filterInit.stateNoiseNum;
                        end 
                    end
