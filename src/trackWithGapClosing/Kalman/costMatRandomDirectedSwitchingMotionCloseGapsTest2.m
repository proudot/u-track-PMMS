function [costMat,nonlinkMarker,indxMerge,numMerge,indxSplit,numSplit,...
    errFlag] = costMatRandomDirectedSwitchingMotionCloseGapsTest2(trackedFeatInfo,...
...
for iPair = 1 : numPairs
    ...
    endTime = trackEndTime(iEnd);
    startTime = trackStartTime(iStart);
    ...
    if possibleLink
        ...
        %calculate the average displacement of each track
        ...
        if trackTypeE == 1 && trackTypeS == 1
            ...
            cost12 = mean( [dispVecMag/(timeScalingLin(timeGap)*meanDispTrack1) ...
                dispVecMag/(timeScalingLin(timeGap)*meanDispTrack2)] ).^2 ...
                * (1 + mean([sin2Angle sin2AngleE sin2AngleS]));
        elseif trackTypeE == 1
            %             cost12 = dispVecMag2 * (1 + sin2AngleE) ...
            %                 / (mean([timeScalingLin(timeGap)*meanDispTrack2 ...
            %                 timeScalingBrown(timeGap)*meanDispTrack1]))^2;
            cost12 = mean( [dispVecMag/(timeScalingLin(timeGap)*meanDispTrack2) ...
                dispVecMag/(timeScalingBrown(timeGap)*meanDispTrack1)] ).^2 ...
                * (1 + sin2AngleE);
        elseif trackTypeS == 1
            ...
            cost12 = mean( [dispVecMag/(timeScalingLin(timeGap)*meanDispTrack1) ...
                dispVecMag/(timeScalingBrown(timeGap)*meanDispTrack2)] ).^2 ...
                * (1 + sin2AngleS);
        else
            %             cost12 = dispVecMag2 ...
            %                 / (timeScalingBrown(timeGap) * meanDisp2Tracks)^2;
            cost12 = mean( [dispVecMag/(timeScalingBrown(timeGap)*meanDispTrack1) ...
                dispVecMag/(timeScalingBrown(timeGap)*meanDispTrack2)] ).^2;
        end
        cost12 = cost12 * ampRatio; %amplitude contribution added later since it is the same for all scenarios
        ...
        for endTime = 1 : numFrames-1
            ...
            for iPair = 1 : numPairs
                ...
                if possibleLink
                    ...
                    meanDispTrack1 = trackMeanDispMag(iEnd); %for displacement scaling
                    if isnan(meanDispTrack1)
                        meanDispTrack1 = meanDispAllTracks;
                    end
                    cost12 = (dispVecMag/meanDispTrack1)^2 * ampCost ...
                        * (1 + sin2AngleE); %cost
        ...
        for startTime = 2 : numFrames
            ...
            for iPair = 1 : numPairs
                ...
                if possibleLink
                    ...
                    meanDispTrack1 = trackMeanDispMag(iStart); %for displacement scaling
                    if isnan(meanDispTrack1)
                        meanDispTrack1 = meanDispAllTracks;
                    end
                    cost12 = (dispVecMag/meanDispTrack1)^2 * ampCost ...
...
prctile2use = 100;
costBD = 1.05*max(prctile(cost(:),prctile2use),eps);
