function [costMat,propagationScheme,kalmanFilterInfoFrame2,nonlinkMarker,...
    errFlag] = costMatEndosomesLink(movieInfo,kalmanFilterInfoFrame1,...
...
searchRadius = repmat(searchRadius,[1 1 3]);

%assign NaN to costs corresponding to distance > searchRadius
costMatTmp(costMatTmp>searchRadius) = NaN;

%% Penalty for switching propagation schemes

%define penalty for switching
switchPenalty = 2;

%get propagation shemes of features in first frame
currentScheme = kalmanFilterInfoFrame1.scheme(:,1);

%go over all features in first frame
for iFeature = 1 : numFeaturesFrame1
    
    %get this feature's scheme
    featureScheme = currentScheme(iFeature);
    
    %if this feature has a scheme (i.e. it's not a new particle tracked
    %for the first time)
    if featureScheme ~= 0
        
        %get this feature's linking costs
        costFeature = costMatTmp(iFeature,:,:);
        
        %get the costs of links resulting from this feature's current scheme
        costForScheme = costFeature(1,:,featureScheme);
        
        %if there is at least one link resulting from the current
        %scheme, then penalize the links resulting from the other
        %schemes
        if any(~isnan(costForScheme))
            notFeatureScheme = setdiff(1:3,featureScheme);
            costFeature(1,:,notFeatureScheme) = ...
                costFeature(1,:,notFeatureScheme) * switchPenalty;
        end
        
        %also do not allow instantaneous switching between forward and
        %backward movement, i.e. there has to be at least one brownian
        %step before switching direction
        if featureScheme==1 || featureScheme==2
            costFeature(1,:,setdiff(1:2,featureScheme)) = NaN;
        end
        
        %put costs back in cost matrix
        costMatTmp(iFeature,:,:) = costFeature;
        
    else %if this feature is a new particle
        
        %get this feature's linking costs
        costFeature = costMatTmp(iFeature,:,:);
        
        %penalize the links so that all linking costs are at the same
        %level
        costFeature(1,:,:) = costFeature(1,:,:) * switchPenalty;
