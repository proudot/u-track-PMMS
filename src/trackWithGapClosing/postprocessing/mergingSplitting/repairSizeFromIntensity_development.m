function [segmentSetOUT,notRepIntSizeErr] = repairSizeFromIntensity_development(...
    segID,segmentSetIN,checkAll,notRepIntSizeErr,intensityInfo)
%REPAIRSIZEFROMINTENSITY_DEVELOPMENT is a latest stable development code
%for repairSizeFromIntensity. This function corrects errors in segment 
%sizes that were determined from intensity amplitudes. This function 
%performs repairs recursively.
%
...
%   
    ...
    elseif ( ~isnan(segmentSetIN(segID,3)) && ~isnan(segmentSetIN(segID,4)) )
        ...
        if ( (child1Size + child2Size ~= parentSize) ||... 
            ...
            [segmentSetOUT,notRepIntSizeErr] = repairSizeFromIntensity_development(child1ID,segmentSetIN,0,notRepIntSizeErr,intensityInfo);
            ...
            if ( (child1Size_rev + child2Size ~= parentSize) ||... 
                ...
                if ( segmentSetIN(child2ID,3)~= segmentSetIN(child1ID,3))
                    [segmentSetOUT,notRepIntSizeErr] = repairSizeFromIntensity_development(child2ID,segmentSetOUT,0,notRepIntSizeErr,intensityInfo);
                end
                    ...
                    if (~swapFxd)
                        ...
                        elseif (isnan(parentChk) && isnan(segmentSetOUT(child1ID,3)) && ...
                            ...
                            if (tempChild2Chk == 1)
                                ...
                                %child not ok.
                                if ( (parentSize > child1Size_rev) &&...
                                        ((child1Size_rev > 1) || (parentSize - child1Size_rev > 1)) )
                                    segmentSetOUT(child2ID,8) = parentSize - child1Size_rev;
                                    if (checkSegmentSize(child2ID,segmentSetOUT,1,1,0) == 1)
                                        tempRepFlag = 1;
                                    else
                        ...
                        elseif (isnan(parentChk) && ~isnan(segmentSetOUT(child1ID,3)) && ...
                            ...
                            if (tempChild1Chk == 1)
                                ...
                                if ( (parentSize > child2Size_rev) &&...
                                        ((child2Size_rev > 1) || (parentSize - child2Size_rev > 1)) )
                                    segmentSetOUT(child1ID,8) = parentSize - child2Size_rev;
                                    if (checkSegmentSize(child1ID,segmentSetOUT,1,1,0) == 1)
                                        tempRepFlag = 1;
                                    else
                                        %Update failed. Revert.
                        ...
                        elseif (isnan(parentChk) && ~isnan(segmentSetOUT(child1ID,3)) && ...
                            ...
                            if ( (tempChild1Chk == 1) && (tempChild2Chk == 1) )
                                if ((child1Size_rev == 1) || (child2Size_rev == 1))
                                    ...
                                    [segmentSetOUT,notRepIntSizeErr] =...
                                        repairSizeFromIntensity_development(smallerChildID,segmentSetOUT,0,...
                                        notRepIntSizeErr,intensityInfo);
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                end
                            elseif( (tempChild1Chk ~= 1) && (tempChild2Chk == 1) )
                                ...
                                if ((child1Size_rev > 1) && (child2Size_rev > 1))
                                    ...
                                    %Here ((child1Size_rev == 1) || (child2Size_rev == 1))                                    
                                    if ( (parentSize > child2Size_rev) &&...
                                            ((child2Size_rev > 1) || (parentSize - child2Size_rev > 1)) )                                    
                                        %Try updating child1.
                                        segmentSetOUT(child1ID,8) = parentSize - child2Size_rev;
                                    ...
                                    if (~tempRepFlag)
                                        ...
                                        childInts = [segmentSetIN(child1ID,7);segmentSetIN(child2ID,7)];
                                        %tempBigger = find((childInts == max(childInts)));
                                        biggerChildID = childIDs((childInts == max(childInts)));
                                        smallerChildID = childIDs(childIDs ~= biggerChildID);                                        
                                        ...
                                        %Update child 1.
                                        if (smallerChildID(1) == child2ID)
                                            segmentSetOUT(biggerChildID(1),8) = newParentSize - child2Size_rev;
                                        else
                                            segmentSetOUT(smallerChildID(1),8) = newParentSize - child2Size_rev;
                                        end
                                        tempRepFlag = 1;
                                    end
                            ...
                            elseif( (tempChild1Chk == 1) && (tempChild2Chk ~= 1) )
                                if ((child1Size_rev > 1) && (child2Size_rev > 1))
                                    ...
                                    if (~tempRepFlag)
                                        ...
                                        segmentSetOUT(parentID,8) = newParentSize;
                                        %Update child 2.
                                        if (smallerChildID(1) == child1ID)
                                            segmentSetOUT(biggerChildID(1),8) = newParentSize - child1Size_rev;
                                        else
                                            segmentSetOUT(smallerChildID(1),8) = newParentSize - child1Size_rev;
                                        end
                                        tempRepFlag = 1;
                                    end
                            ...
                            elseif( (tempChild1Chk ~= 1) && (tempChild2Chk ~= 1) )
                                %Child size conditions do not matter here.
                                %The lower one must be set to 1.
                                segmentSetOUT = assignSizeFromProb([parentID;NaN],parentSize,...
                                    [child1ID;child2ID],segmentSetIN,segmentSetOUT,intensityInfo);                                  
                                %No need to recall with child segs here
                                %because checks = 0 for both.
                        ...
                        elseif (parentChk == 1)
                            ...
                            if (~isnan(segmentSetOUT(child1ID,3)) && ~isnan(segmentSetOUT(child2ID,3)) )
                                ...
                                elseif ((tempChild1Chk ~= 1) && (tempChild2Chk ~= 1))
                                    ...
                                    %child1Chk = 1 and/or child2Chk = 1.
                                    childIDs = [child1ID;child2ID];
                                    %childSizes = [child1Size_rev;child2Size_rev];
                                    childInts = [segmentSetIN(child1ID,7);segmentSetIN(child2ID,7)];
                                    %tempBigger = find((childInts == max(childInts)));
                                    ...
                                    if (parentSize > 1)
                                        segmentSetOUT(biggerChildID(1),8) = parentSize - 1;
                                    elseif (parentSize == 1)
                                        %parentChk = 1 is incorrect
                                        segIntensity = [segmentSetIN(parentID,7);segmentSetIN(biggerChildID(1),7)];
                                        newParentSize = getSizeFromProb(parentSize,segIntensity,intensityInfo);
                                ...
                                elseif ( (tempChild1Chk ~= 1) && (tempChild2Chk == 1) ) 
                                    %Child 2 is ok. Fix child 1 and parent
                                    %if necessary.
                                    if (child2Size_rev > 1)
                                        segmentSetOUT(child1ID,8) = 1;
                                        if (parentSize == 1)
                                ...
                                elseif ( ((tempChild1Chk == 1) && (tempChild2Chk ~= 1)) )
                                    %Child 1 is ok. Fix child 2 and parent
                                    %if necessary.
                                    if (child1Size_rev > 1)
                                        segmentSetOUT(child2ID,8) = 1;
                                        if (parentSize == 1)
                                    ...
                                    elseif (child1Size_rev == 1)
                                        if (parentSize == 1)
                                            %parentChk = 1 is incorrect
                                            segIntensity = [segmentSetIN(parentID,7);segmentSetIN(child2ID,7)];
                                            newParentSize = getSizeFromProb(parentSize,segIntensity,intensityInfo);
                                            segmentSetOUT(parentID,8) = newParentSize;
                                ...
                                if (segmentSetOUT(parentID,8) ~= parentSize)
                                    ...
                                    if (segmentSetOUT(parentID,10) == 1)
                                        ...
                                        [segmentSetOUT,notRepIntSizeErr] =...
                                            repairSizeFromIntensity_development(segmentSetOUT(parentID,1),segmentSetOUT,0,...
                                            notRepIntSizeErr,intensityInfo);
                                ...
                                if ((tempChild1Chk == 1) &&...
                                    ...
                                    [segmentSetOUT,notRepIntSizeErr] =...
                                        repairSizeFromIntensity_development(child1ID,segmentSetOUT,0,...
                                        notRepIntSizeErr,intensityInfo);
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                end   
                                if ((tempChild2Chk == 1) &&...
                                    ...
                                    [segmentSetOUT,notRepIntSizeErr] =...
                                        repairSizeFromIntensity_development(child2ID,segmentSetOUT,0,...
                                        notRepIntSizeErr,intensityInfo);
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                end                                  
        ...
        elseif (checkAll && (child1Size + child2Size == parentSize))
            ...
            [segmentSetOUT,notRepIntSizeErr] = repairSizeFromIntensity_development(segmentSetIN(segID,3),...
                segmentSetIN,1,notRepIntSizeErr,intensityInfo);
            %Continue on with child 2
            [segmentSetOUT,notRepIntSizeErr] = repairSizeFromIntensity_development(segmentSetOUT(segID,4),...
                segmentSetIN,1,notRepIntSizeErr,intensityInfo);            
        else
            %No problem
            %{
    ...
    elseif (~isnan(segmentSetIN(segID,3)) && ~isnan(segmentSetIN(segID,6)) )
        ...
        if ( (childSize ~= parent1Size + parent2Size) ||...
            ...
            [segmentSetOUT,notRepIntSizeErr] = repairSizeFromIntensity_development(childID,segmentSetIN,0,notRepIntSizeErr,intensityInfo);
                ...
                if (~swapFxd)
                        ...
                        if (tempChildChk ~= 1)
                            ...
                            if ( (parent1Size == 1) || (parent2Size == 1))
                                ...
                                if(checkSegmentSize(childID,segmentSetOUT,1,1,0) ~= 1)
                                    ...
                                    segmentSetOUT(childID,8) = childSize_rev;
                                end
                            end
                            
                            if (~tempRepFlag)
                                %if p1Size > 1 && p2Size > 1 OR the above
                                %failed.
                                segmentSetOUT = assignSizeFromProb([parent1ID;parent2ID],childSize_rev,...
                                    [childID;NaN],segmentSetIN,segmentSetOUT,intensityInfo);                                       
                            ...
                            elseif (childSize_rev == 1)
                                ...
                                %if (segmentSetOUT(childID,8) ~= childSize_rev)
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                                    [segmentSetOUT,notRepIntSizeErr] =...
                                        repairSizeFromIntensity_development(childID,segmentSetOUT,0,...
                                        notRepIntSizeErr,intensityInfo);
                                    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
                                %end                                   
                            end
                        ...
                        if (isnan(segmentSetIN(childID,3)))
                            ...
                            if ( ((parent1Size == 1) || (parent2Size == 1)) )
                                ...
                                if (isnan(segmentSetOUT(smallerParentID(1),10)))
                                    ...
                                    %Recall for parent with its parent
                                    [segmentSetOUT,notRepIntSizeErr] =...
                                        repairSizeFromIntensity_development(segmentSetOUT(smallerParentID(1),1),segmentSetOUT,0,...
                                        notRepIntSizeErr,intensityInfo);
                                else
                            ...
                            if (tempChildChk ~= 1)
                                %Child not ok.
                                %Update child if parent sizes ok.
                                if ( (parent1Size == 1) || (parent2Size == 1))
                                    segmentSetOUT(childID,8) = parent1Size + parent2Size;
                                    tempRepFlag = 1;
                                else
                                    %Set smaller parent to 1 and recall.
                                    %Other parent is ok. So, no need to
                                    %call getSizeFromProb.
                                    parentIDs = [parent1ID;parent2ID];                            
                                    parentInts = [segmentSetIN(parent1ID,7);segmentSetIN(parent2ID,7)];                            
                                    biggerParentID = parentIDs((parentInts == max(parentInts)));
                                    smallerParentID = parentIDs(parentIDs ~= biggerParentID(1));
                    ...
                    elseif ( isnan(parent1Chk) && (parent2Chk == 0) && ...
                        ...
                        if (tempChildChk == 1) 
                            ...
                            %Call child too, since behind it?
                        end
...
%       Modified, July 2014
    ...
    if (isnan(parentID(2)))
        ...
        %merge
        if (tempRepFlag)                        
            outStr = sprintf('(DEV) Repaired (%s): (%d:%d) + (%d:%d) -> (%d:%d). ',caseStr,...
